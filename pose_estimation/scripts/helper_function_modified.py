...
class ICPTransform():
    ...
    def least_squares_transform(self, scene, new_model) -> RigidTransform:
        ...
        # Ensure rotation respects symmetry (e.g., only 90-degree increments)
        Rstar = self.align_axis_to_z(Rstar)
    ...
    def ICP(self, scene, case):
        ...
        X_BA, initia_X_BA, _, _ = self.icp(scene, case)
    ...
    def align_axis_to_z(self, R_obj, symmetry_axes=None):
        """
        Align the axis of the object to the world z-axis, considering symmetry.

        Args:
        - R_obj (numpy.ndarray): A 3x3 rotation matrix representing the object frame.
        - symmetry_axes (list): List of symmetry axis indices (e.g., [0, 1, 2] for all three).

        Returns:
        - R_aligned (numpy.ndarray): A rotation matrix with the best alignment.
        """
        ...
        for axis in symmetry_axes:
            obj_axis = R_obj[:, axis]
            cross_prod = np.cross(obj_axis, z_world)
            cross_prod_norm = np.linalg.norm(cross_prod)
            angle = np.arctan2(cross_prod_norm, np.dot(obj_axis, z_world))
            if angle < min_angle:
                min_angle = angle
                best_rotation = R.from_rotvec(cross_prod * angle).as_matrix() @ R_obj
...
if __name__ ==  '__main__':
    ...
    X_BA, initial_X_BA = process.ICP(scene, case)
